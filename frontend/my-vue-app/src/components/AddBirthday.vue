  ...
  methods: {
    ...
    async submitForm() {
      try {
        ...
        const response = await axios.post('http://localhost:8080/', birthdayData, {
          headers: {
            'Content-Type': 'application/json',
          },
        });

        console.log('День рождения добавлен:', response.data);
      } catch (error) {
        console.error('Ошибка при добавлении дня рождения:', error);
      }
    },
  },
};
